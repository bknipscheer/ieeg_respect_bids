...
countcurr = 1; countstim=1;countlow = 1;
...
for i=numannots
    ...
    if ~isempty(regexp(lower(annots_new{i,2}),'_[1-9]+ma', 'once'))
        
        annot_curr(countcurr,1) = annots_new{i,1};
        annot_curr(countcurr,2) = str2double(cell2mat(extractBetween(regexp(lower(annots_new{i,2}),'_[1-9]+ma','match'),'_','ma')));
        countcurr = countcurr+1;
    end
    
    if ~isempty(regexp(lower(annots_new{i,2}),'sec', 'once'))
        annot_pulsdur = [annot_pulsdur;annots_new{i,1}];
    end
        
    if ~isempty(regexp(lower(annots_new{i,2}),'hz', 'once'))
        annot_freq = [annot_freq;annots_new{i,1}];
    end
