...
[low_cutoff{:}]                     = deal(cfg(1).HardwareFilters.LowpassFilter.CutoffFrequency);%{header.elec(:).Prefiltering_LowPass_Limit}'                                             ;
high_cutoff                         = cell(size(name)) ;
[high_cutoff{:}]                    = deal(cfg(1).HardwareFilters.HighpassFilter.CutoffFrequency);%{header.elec(:).Prefiltering_HiPass_Limit}'                                              ;
